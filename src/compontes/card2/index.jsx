import { Card, CardHeader, CardBody, Typography, Avatar, Button } from "@material-tailwind/react";

function TechCard() {
  return (
    <Card className="w-96 shadow-lg">
      <CardHeader color="blue-gray" className="relative h-48">
        <img
          src="https://a.d-cd.net/o6YewbS4QlTdEydh7RaC-Usf3zo-960.jpg"
          alt="tech-image"
          className="w-full h-full object-cover"
        />
      </CardHeader>
      <CardBody className="text-center">
        <Button size="sm" color="light-blue" className="rounded-full mb-2">
          Technology
        </Button>
        <Typography variant="h5" color="blue-gray" className="mb-2">
          What's New In 2022 Tech
        </Typography>
        <Typography className="text-gray-700 text-sm mb-4">
          Lorem ipsum dolor sit amet consectetur adipisicing elit. Sequi
          perferendis molestiae non nemo dolorum.
        </Typography>
        <div className="flex items-center justify-center">
          <Avatar src="data:image/jpeg;base64,/100x100" alt="author" size="sm" />
          <div className="ml-3">
            <Typography variant="small" className="text-gray-700">Jane Doe</Typography>
            <Typography variant="small" className="text-gray-500">2h ago</Typography>
          </div>
        </div>
      </CardBody>
    </Card>
  );
}

export default TechCard;
